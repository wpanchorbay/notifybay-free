...
if ( $plain_text ) {
	...
	echo esc_html( sprintf( __( 'The item %s you were waiting for is now available!', 'notifybay-waitlist-and-stock-alert-woo' ), $product_name ) ) . "\n\n";
	if ( $buy_link ) {
		echo esc_html__( 'Buy now:', 'notifybay-waitlist-and-stock-alert-woo' ) . ' ' . esc_url( $buy_link ) . "\n\n";
	}
	if ( ! empty( $context['unsubscribe_url'] ) ) {
		echo esc_html__( 'Unsubscribe:', 'notifybay-waitlist-and-stock-alert-woo' ) . ' ' . esc_url( $context['unsubscribe_url'] ) . "\n";
	}
	return;
}
