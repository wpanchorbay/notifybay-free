...
if ( $plain_text ) {
	...
	echo esc_html( sprintf( __( 'Please confirm you want to receive notifications for %s.', 'notifybay' ), $product_name ) ) . "\n\n";
	if ( $verify_url ) {
		echo esc_html__( 'Confirm:', 'notifybay' ) . ' ' . esc_url( $verify_url ) . "\n";
	}
	return;
}
...
<p>
	<?php
	/* translators: %s: product name */
	printf( esc_html__( 'Please confirm you want to receive notifications for %s.', 'notifybay' ), '<strong>' . esc_html( $product_name ) . '</strong>' );
	?>
</p>
...
<p>
	<a href="<?php echo esc_url( $verify_url ); ?>" style="background:#000; color:#fff; padding:10px 20px; text-decoration:none; display:inline-block; border-radius:5px;">
		<?php esc_html_e( 'Confirm Subscription', 'notifybay' ); ?>
	</a>
</p>
<?php endif; ?>
